...
export function ShareModal({
  ...
  const handleSocialShare = (platform: string) => {
    ...
    const copyToClipboard = (text: string) => {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard
          .writeText(text)
          .then(() => {
            toast.success(`Link copied to clipboard. Share it on ${platform}`);
          })
          .catch((err) => {
            console.error("Could not copy text: ", err);
    ...
    switch (platform) {
      case "facebook":
        // Facebook standard sharer with properly URL-encoded parameters
        const facebookShareUrl = `https://www.facebook.com/sharer.php?u=${encodedUrl}&t=${encodedTitle}`;

        // Open in a new window using Facebook's preferred dimensions
        const fbWidth = 626;
        const fbHeight = 436;
        const fbLeft = (window.innerWidth - fbWidth) / 2;
        const fbTop = (window.innerHeight - fbHeight) / 2;

        window.open(
          facebookShareUrl,
          "facebook-share-dialog",
          `width=${fbWidth},height=${fbHeight},left=${fbLeft},top=${fbTop},toolbar=0,location=0,menubar=0,directories=0,scrollbars=0`
        );
        return; // Return early to skip the general window.open at the end
      ...
      case "messenger":
        ...
        break;
      case "wechat":
        // WeChat requires a QR code, but for simplicity we'll just copy the link
        copyToClipboard(shareLink);
        return;
      case "instagram":
        // Instagram doesn't have a direct sharing API, so we just copy to clipboard
        copyToClipboard(shareLink);
        return;
      case "tiktok":
        // TikTok doesn't have a direct sharing API, so we just copy to clipboard
        copyToClipboard(shareLink);
        return;
      default:
        break;
    ...
    if (shareUrl) {
      ...
      // Open a popup window for web sharing
      const width = 600;
      const height = 400;
          ...
          {isPublic && (
            <>
              ...
              {/* <div className="space-y-3">
                <Label className="text-sm font-medium">Share as post</Label>
                <div className="grid grid-cols-4 gap-3 py-2">
                  <Button
                    variant="outline"
                    className="rounded-md h-20 p-2 flex flex-col items-center justify-center gap-2 hover:bg-blue-50 border-2"
                  ...
                  <Button
                    variant="outline"
                    className="rounded-md h-20 p-2 flex flex-col items-center justify-center gap-2 hover:bg-pink-50 border-2"
                  ...
                  <Button
                    variant="outline"
                    className="rounded-md h-20 p-2 flex flex-col items-center justify-center gap-2 hover:bg-black/5 border-2"
              ...
              <div className="space-y-3">
                <Label className="text-sm font-medium">Share as message</Label>
                <div className="grid grid-cols-4 gap-3 py-2">
                  <Button
                    variant="outline"
                    className="rounded-md h-20 p-2 flex flex-col items-center justify-center gap-2 hover:bg-blue-50 border-2"
                  ...
                  <Button
                    variant="outline"
                    className="rounded-md h-20 p-2 flex flex-col items-center justify-center gap-2 hover:bg-green-50 border-2"
