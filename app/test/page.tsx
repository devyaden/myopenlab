...
function Default() {
  ...
  function toolbarClick(args: any) {
    ...
    switch (item) {
      ...
      case "Cut":
        diagramInstance.cut();
        var pasteIndex = toolbarEditor.items.findIndex(
          (item) => item.id === "Paste"
        );
        if (pasteIndex !== -1) {
  ...
  function historyChange(args: any) {
    ...
    if (undoItem) {
      undoItem.disabled =
        diagramInstance.historyManager.undoStack.length > 0 ? false : true;
    }
    const redoItem = toolbarEditor.items.find((item) => item.id === "Redo");
    if (redoItem) {
      redoItem.disabled =
        diagramInstance.historyManager.redoStack.length > 0 ? false : true;
    }
  }
  return (
              ...
              selectionChange={(args: ISelectionChangeEventArgs) => {
                if (args.state === "Changed") {
                  ...
                  if (selectedItems.length === 0) {
                    ...
                    itemIds.forEach((itemId) => {
                      const item = toolbarEditor.items.find(
                        (item) => item.id === itemId
                      );
                      if (item) {
                        item.disabled = true;
                  ...
                  if (selectedItems.length === 1) {
                    ...
                    if (
                      ...
                      var Index = toolbarEditor.items.findIndex(
                        (item) => item.id === "Group"
                      );
                      if (Index !== -1) {
                        toolbarEditor.items[Index].disabled = false;
                  ...
                  if (selectedItems.length > 1) {
                    ...
                    if (selectedItems.length > 2) {
                      var Index = toolbarEditor.items.findIndex(
                        (item) => item.id === "Distribute_objects"
                      );
                      if (Index !== -1) {
                        toolbarEditor.items[Index].disabled = false;
                      }
